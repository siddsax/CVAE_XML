...
def train(x_tr, y_tr, x_te, y_te, x_20, y_20, embedding_weights, params, decoder_word_input=None, decoder_target=None, decoder_word_input_t=None, decoder_target_t=None):
	...
	for epoch in range(params.num_epochs):
			...
			if i % int(100) == 0:
				if(torch.__version__ == '0.4.0'):
						print('Iter-{}; Loss: {:.4}; best_loss: {:.4}; max_grad: {}'.format(i,
							loss.data, loss_best2, max_grad))
				else:
						print('Iter-{}; Loss: {:.4}; best_loss: {:.4}; max_grad: {}'.format(i, loss.data[0], loss_best2[0], max_grad[0]))
		...
		print('End-of-Epoch: Loss: {:.4}; best_loss: {:.4};'.format(loss_epch, best_epch_loss))
		...
		if(test_prec_acc > best_test_acc):
			...
			torch.save(state, "saved_models/" + params.model_name + "/model_best_for_test")
